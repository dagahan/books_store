...
def get_user_router(db: DataBase) -> APIRouter:
    ...
    async def register(user_data: UserCreateDTO,
        ...
        try:
            ...
            session_id = sessions_manager.create_session(user.id)
            access_token = jwt_parser.generate_access_token(user.id, session_id)
            refresh_token = jwt_parser.generate_refresh_token(user.id, session_id)
